...
//bodies.push(new Planet(width/2-550, height/2, Math.sqrt(G*2*SUN/550), Math.PI/2, "rocky", 125, 0, 0));
...
bodies.push(new Star(width/2, height/2, 0, 0, "giant"));
for(let i = 0; i < 15; i++) {
    bodies.push(new Planet(width/2 - 50*i - 100, height/2, Math.sqrt(G*bodies.at(0).getMass()/(50*i+100)), Math.PI/2, "rocky", 100+5*i, 0, 0));
    bodies.push(new Planet(width/2 + 50*i + 100, height/2, Math.sqrt(G*bodies.at(0).getMass()/(50*i+100)), 3*Math.PI/2, "rocky", 0, 0, 100+5*i));
}
